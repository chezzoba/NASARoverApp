...
class Document {
    constructor(model, object, settings) {
        ...
        this[internalProperties].originalObject = Object.assign({}, documentObject);
...
Document.objectFromSchema = function (object, model, settings = { "type": "toDynamo" }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (settings.defaults || settings.forceDefault) {
            yield Promise.all(Document.attributesWithSchema(returnObject, model).map((key) => __awaiter(this, void 0, void 0, function* () {
                const value = utils.object.get(returnObject, key);
                if (value === dynamooseUndefined) {
                    utils.object.set(returnObject, key, undefined);
                }
                else {
                    const defaultValue = yield model.schema.defaultCheck(key, value, settings);
        ...
        if (settings.modifiers) {
            yield Promise.all(settings.modifiers.map((modifier) => {
                return Promise.all(Document.attributesWithSchema(returnObject, model).map((key) => __awaiter(this, void 0, void 0, function* () {
                    const value = utils.object.get(returnObject, key);
                    const modifierFunction = yield model.schema.getAttributeSettingValue(modifier, key, { "returnFunction": true });
                    const isValueUndefined = typeof value === "undefined" || value === null;
        ...
        if (settings.validate) {
            yield Promise.all(Document.attributesWithSchema(returnObject, model).map((key) => __awaiter(this, void 0, void 0, function* () {
                ...
                const isValueUndefined = typeof value === "undefined" || value === null;
                if (!isValueUndefined) {
                    const validator = yield model.schema.getAttributeSettingValue("validate", key, { "returnFunction": true });
                    if (validator) {
                        let result;
        ...
        if (settings.required) {
            ...
            yield Promise.all(attributesToCheck.map((key) => __awaiter(this, void 0, void 0, function* () {
                const check = () => __awaiter(this, void 0, void 0, function* () {
                    const value = utils.object.get(returnObject, key);
                    yield model.schema.requiredCheck(key, value);
                });
        ...
        if (settings.enum) {
            yield Promise.all(Document.attributesWithSchema(returnObject, model).map((key) => __awaiter(this, void 0, void 0, function* () {
                ...
                const isValueUndefined = typeof value === "undefined" || value === null;
                if (!isValueUndefined) {
                    const enumArray = yield model.schema.getAttributeSettingValue("enum", key);
...
Document.prototype.conformToSchema = function (settings = { "type": "fromDynamo" }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const expectedObject = yield Document.objectFromSchema(this, this.model, settings);
        if (!expectedObject) {
