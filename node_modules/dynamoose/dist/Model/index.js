...
// Utility functions
function getTableDetails(model, settings = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        const func = () => __awaiter(this, void 0, void 0, function* () {
            const tableDetails = yield ddb("describeTable", { "TableName": model.name });
...
function createTableRequest(model) {
    return __awaiter(this, void 0, void 0, function* () {
        return Object.assign(Object.assign({ "TableName": model.name }, utils.dynamoose.get_provisioned_throughput(model.options)), yield model.schema.getCreateTableAttributeParams(model));
    });
}
function createTable(model) {
    return __awaiter(this, void 0, void 0, function* () {
        if ((((yield getTableDetails(model, { "allowError": true })) || {}).Table || {}).TableStatus === "ACTIVE") {
            model.alreadyCreated = true;
            return () => Promise.resolve.bind(Promise)();
        }
        yield ddb("createTable", yield createTableRequest(model));
    });
}
function updateTimeToLive(model) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        function updateDetails() {
            return __awaiter(this, void 0, void 0, function* () {
                ttlDetails = yield ddb("describeTimeToLive", {
        ...
        function check(count) {
            ...
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    const tableDetails = (yield getTableDetails(model, { "forceRefresh": forceRefreshOnFirstAttempt === true ? forceRefreshOnFirstAttempt : count > 0 })).Table;
                ...
                if (count > 0) {
                    model.options.waitForActive.check.frequency === 0 ? yield utils.set_immediate_promise() : yield utils.timeout(model.options.waitForActive.check.frequency);
                }
...
function updateTable(model) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (updateAll || model.options.update.includes(ModelUpdateOptions.throughput)) {
            const currentThroughput = (yield getTableDetails(model)).Table;
            ...
            if (!isThroughputUpToDate) {
                ...
                yield ddb("updateTable", object);
                yield waitForActive(model)();
            }
        }
        // Indexes
        if (updateAll || model.options.update.includes(ModelUpdateOptions.indexes)) {
            const tableDetails = yield getTableDetails(model);
            const existingIndexes = tableDetails.Table.GlobalSecondaryIndexes;
            const updateIndexes = yield utils.dynamoose.index_changes(model, existingIndexes);
            yield updateIndexes.reduce((existingFlow, index) => __awaiter(this, void 0, void 0, function* () {
                yield existingFlow;
                const params = {
                    "TableName": model.name
                };
                if (index.type === index_changes_1.ModelIndexChangeType.add) {
                    params.AttributeDefinitions = (yield model.schema.getCreateTableAttributeParams(model)).AttributeDefinitions;
                    params.GlobalSecondaryIndexUpdates = [{ "Create": index.spec }];
                }
                else {
                    params.GlobalSecondaryIndexUpdates = [{ "Delete": { "IndexName": index.name } }];
                }
                yield ddb("updateTable", params);
                yield waitForActive(model)();
            }), Promise.resolve());
        }
    });
...
class Model {
    constructor(name, schema, options) {
        ...
        if (!schema) {
            ...
            schema = new Schema_1.Schema(schema);
        }
        if (options.expires) {
        ...
        this.Document.transaction = [
            ...
            { "key": "condition", "settingsIndex": -1, "dynamoKey": "ConditionCheck", "function": (key, condition) => (Object.assign({ "Key": this.Document.objectToDynamo(convertObjectToKey.bind(this)(key)), "TableName": this.name }, condition ? condition.requestObject() : {})) }
            ...
            accumulator[key] = (...args) => __awaiter(this, void 0, void 0, function* () {
    ...
    batchGet(keys, settings, callback) {
        ...
        const prepareResponse = (response) => __awaiter(this, void 0, void 0, function* () {
            const tmpResult = yield Promise.all(response.Responses[this.name].map((item) => documentify(item)));
            const unprocessedArray = response.UnprocessedKeys[this.name] ? response.UnprocessedKeys[this.name].Keys : [];
            const tmpResultUnprocessed = yield Promise.all(unprocessedArray.map((item) => this.Document.fromDynamo(item)));
            const startArray = Object.assign([], { "unprocessedKeys": [] });
    ...
    batchPut(documents, settings, callback) {
        ...
        const prepareResponse = (response) => __awaiter(this, void 0, void 0, function* () {
            ...
            const tmpResultUnprocessed = yield Promise.all(unprocessedArray.map((item) => this.Document.fromDynamo(item.PutRequest.Item)));
        ...
        const paramsPromise = (() => __awaiter(this, void 0, void 0, function* () {
            return ({
                "RequestItems": {
                    [this.name]: yield Promise.all(documents.map((document) => __awaiter(this, void 0, void 0, function* () {
                        return ({
                            "PutRequest": {
                                "Item": yield new this.Document(document).toDynamo({ "defaults": true, "validate": true, "required": true, "enum": true, "forceDefault": true, "saveUnknown": true, "customTypesDynamo": true, "updateTimestamps": true, "modifiers": ["set"] })
                            }
                        });
                    })))
                }
    ...
    batchDelete(keys, settings, callback) {
        ...
        const prepareResponse = (response) => __awaiter(this, void 0, void 0, function* () {
            const unprocessedArray = response.UnprocessedItems && response.UnprocessedItems[this.name] ? response.UnprocessedItems[this.name] : [];
            const tmpResultUnprocessed = yield Promise.all(unprocessedArray.map((item) => this.Document.fromDynamo(item.DeleteRequest.Key)));
            return keyObjects.reduce((result, key) => {
    ...
    update(keyObj, updateObj, settings, callback) {
        ...
        const getUpdateExpressionObject = () => __awaiter(this, void 0, void 0, function* () {
            const updateTypes = [
            ...
            const returnObject = yield Object.keys(updateObj).reduce((accumulatorPromise, key) => __awaiter(this, void 0, void 0, function* () {
                ...
                for (let i = 0; i < valueKeys.length; i++) {
                    ...
                    if (!updateType.attributeOnly && subValue !== dynamooseUndefined) {
                        subValue = (yield this.Document.objectFromSchema({ [subKey]: dynamoType === "L" && !Array.isArray(subValue) ? [subValue] : subValue }, this, Object.assign({ "type": "toDynamo", "customTypesDynamo": true, "saveUnknown": true }, updateType.objectFromSchemaSettings)))[subKey];
                    }
                    ...
                    if (subValue !== dynamooseUndefined) {
                        const defaultValue = yield this.schema.defaultCheck(subKey, undefined, updateType.objectFromSchemaSettings);
                        if (defaultValue) {
                    ...
                    if (updateType.objectFromSchemaSettings.required === true) {
                        yield this.schema.requiredCheck(subKey, undefined);
                    }
                ...
                const defaultObjectFromSchema = yield this.Document.objectFromSchema(this.Document.prepareForObjectFromSchema({}, this, documentFunctionSettings), this, documentFunctionSettings);
            ...
            yield Promise.all(this.schema.attributes().map((attribute) => __awaiter(this, void 0, void 0, function* () {
                const defaultValue = yield this.schema.defaultCheck(attribute, undefined, { "forceDefault": true });
        ...
        const documentify = (document) => new this.Document(document, { "type": "fromDynamo" }).conformToSchema({ "customTypesDynamo": true, "checkExpiredItem": true, "type": "fromDynamo" });
        const localSettings = settings;
        const updateItemParamsPromise = this.pendingTaskPromise().then(() => __awaiter(this, void 0, void 0, function* () {
            return (Object.assign(Object.assign({ "Key": this.Document.objectToDynamo(keyObj), "ReturnValues": "ALL_NEW" }, utils.merge_objects.main({ "combineMethod": "object_combine" })(localSettings.condition ? localSettings.condition.requestObject({ "index": { "start": index, "set": (i) => {
                        index = i;
                    } }, "conditionString": "ConditionExpression", "conditionStringType": "string" }) : {}, yield getUpdateExpressionObject())), { "TableName": this.name }));
        }));
        if (settings.return === "request") {
            if (callback) {
                const localCallback = callback;
        ...
        if (callback) {
            ...
            return (() => __awaiter(this, void 0, void 0, function* () {
                const response = yield promise;
                return response.Attributes ? yield documentify(response.Attributes) : undefined;
            }))();
        }
    }
    ...
    get(key, settings, callback) {
        ...
        if (callback) {
            ...
            return (() => __awaiter(this, void 0, void 0, function* () {
                const response = yield promise;
                return response.Item ? yield documentify(response.Item) : undefined;
            }))();
        }
    }
}
